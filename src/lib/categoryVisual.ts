 ...
 * we never render lucide on mobile, so each name maps to one emoji + one candy
 * color here. Resolves in three steps: exact `icon_name` → keyword match on the
 * category name → neutral fallback.
 *
